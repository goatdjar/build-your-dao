...
actor {
    ...
    public shared ({ caller }) func createProposal(content : ProposalContent) : async Result<ProposalId, Text> {
        switch (members.get(caller)) {
            case (null) {
                return #err("The caller is not a member - cannot create proposal");
            };
            case (?member) {
                let balance = Option.get(ledger.get(caller), 0);
                if (balance < 1) {
                    return #err("The caller token does not have enough tokens to create a proposal");
                };
                // Create a proposal and burn the tokens
    ...
    // helper functions for voteProposal implementation
